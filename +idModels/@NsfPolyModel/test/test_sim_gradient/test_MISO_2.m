...
u1 = kron(randn(1,20),ones(1,10))'; 
u2 = kron(randn(1,20),ones(1,10))';
u3 = kron(randn(1,20),ones(1,10))';
u = [u1 u2 u3];
...
f(1).input_idx = [1 3];
f(1).output_idx = [1];
f(1).fun = @(u,p,y) deal( u(:,1).*(p(1)-y)+u(:,2),...
                        [],...
                        [p(1)-y ones(size(u,1),1)],...
                        -u(:,1));
...
f(2).input_idx = [1 2 3];
f(2).output_idx = 1;
f(2).fun = @(u,p,y) deal(   p(1).*(u(:,1) - u(:,2).*y).*u(:,3).^2,...
                            [],...
                            [p(1).*u(:,3).^2, -p(1).*y.*u(:,3).^2, 2*p(1).*(u(:,1)-u(:,2).*y).*u(:,3)],...  %%%%%%%%%%%%%%
                            -p(1).*u(:,2).*u(:,3).^2); 
...
f(3).input_idx = [2 3];
f(3).output_idx = [1];
f(3).fun = @(u,p,y) deal( u(:,1).*y + u(:,2).^2,...
                          [],...
                          [y 2.*u(:,2)],...   %%%%%%%%%%%%%%%
                          u(:,1)); 
...
u_grads=[1 3];
[y,x,dy_du] = M1.simulate(u,'CalcInputGradients',u_grads); % Compute Gradient wrt u1 and u3 
...
for k = u_grads
    ...
    Dif = grad -squeeze(dy_du(:,1,i,j)); % shoul be very small (>1e-6)
