...
u1 = kron(randn(1,10),ones(1,20))'; 
u2 = kron(randn(1,10),ones(1,20))';
u3 = kron(randn(1,10),ones(1,20))';
u = [u1 u2 u3];
...
f(2).input_idx = [1 2];
f(2).output_idx = 1;
f(2).fun = @(u,p,y) deal(   p(1).*u(:,1) - u(:,2).*y + p(2),...
                            [],...
                            [p(1)*ones(size(u,1),1) -y],...  %%%%%%%%%%%%%%
                            -u(:,2)); 
...
f(3).input_idx = [2 3];
f(3).output_idx = [];
f(3).fun = @(u,p) deal( u(:,1) + u(:,2).^2,...
                        [],...
                        [ones(size(u,1),1) 2.*u(:,2)],...   %%%%%%%%%%%%%%%
                        []); 
...
u_grads = [1 3];
[y,x,dy_du] = M1.simulate(u,'CalcInputGradients',u_grads); % Compute Gradient wrt u1 and u2
