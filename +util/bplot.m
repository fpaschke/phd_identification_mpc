...
%                        the  bars.
%                        'barwidth'
%       'variablenames': Set specified variablenames as x ticks to the x axis.
% 
%% Jitter feature 
...
if size(x,1)>1 && size(x,2)>1 % great, you want to plot a bunch.
    if isempty(varargin)
        forLegend = util.bplot(x(:,1),1);
        for ii=2:size(x,2)
            hold on;
            util.bplot(x(:,ii),ii,'nolegend');
        end
    else
        if ~ischar(varargin{1})
            warning('You can''t specify a location for multiple guys, this will probably crash');
        end
        forLegend = util.bplot(x(:,1),1,varargin{:});
        for ii=2:size(x,2)
            hold on;
            util.bplot(x(:,ii),ii,'nolegend',varargin{:});
    ...
    %% SET XTICKS
    ix = find(strcmpi(varargin,'variablenames'));
    if ~isempty(ix) 
        VariableNames = varargin{ix+1}; 
        set(gca,'XTick',1:size(x,2),'XTickLabel',VariableNames);
    end
...
%             serrorFlag = 1;
        ...
        case {'variablenames'} % only to make warning disappear
...
if horizontalFlag
    hReg2(end+1) = rectangle('Position',[boxEdge(1),y-barWidth/2,IQR,barWidth],'linewidth',linewidth,'linestyle',linestyle,'EdgeColor',boxColor,'facecolor',[1 1 1]);

    hold on;
    hReg2(end+1) = plot([medianX medianX],[y-barWidth/2 y+barWidth/2],'color',meanColor,'linewidth',linewidth,'linestyle',linestyle);
    if meanFlag
        hReg2(end+1) = plot(meanX,y,'+','color',meanColor,'linewidth',linewidth,'linestyle',linestyle,'markersize',10);
    end
    hReg2(end+1) = plot([boxEdge(1) boxEdge(2)],[y-barWidth/2 y-barWidth/2],'linewidth',linewidth,'linestyle',linestyle,'color',boxColor);

    hReg(end+1) = plot([wisEdge(1) boxEdge(1)],[y y],':','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
    hReg(end+1) = plot([boxEdge(2) wisEdge(2)],[y y],':','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
    hReg2(end+1) = plot([wisEdge(1) wisEdge(1)],[y-barWidth/3 y+barWidth/3],'-','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
    hReg(end+1) = plot([wisEdge(2) wisEdge(2)],[y-barWidth/3 y+barWidth/3],'-','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
else %
    hReg2(end+1) = rectangle('Position',[y-barWidth/2,boxEdge(1),barWidth,IQR],'linewidth',linewidth,'linestyle',linestyle,'EdgeColor',boxColor,'facecolor',[1 1 1]);
    hold on;
    
    hReg2(end+1) = plot([y-barWidth/2 y+barWidth/2],[medianX medianX],'color',meanColor,'linewidth',linewidth,'linestyle',linestyle);
    if meanFlag
        hReg2(end+1) = plot(y,meanX,'+','linewidth',linewidth,'linestyle',linestyle,'color',meanColor,'markersize',10);
    end
    hReg2(end+1) = plot([y-barWidth/2 y-barWidth/2],[boxEdge(1) boxEdge(2)],'linewidth',linewidth,'linestyle',linestyle,'color',boxColor);

    hReg(end+1) = plot([y y],[wisEdge(1) boxEdge(1)],':','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
    hReg(end+1) = plot([y y],[boxEdge(2) wisEdge(2)],':','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
    hReg2(end+1) = plot([y-barWidth/3 y+barWidth/3],[wisEdge(1) wisEdge(1)],'-','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
    hReg(end+1) = plot([y-barWidth/3 y+barWidth/3],[wisEdge(2) wisEdge(2)],'-','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
...
if outlierFlag % but only if you want to
    ...
    if ~isempty(yy)
        ...
        if ~isempty(xx)
            if horizontalFlag
                hReg2(6) = plot(xx,yy,'o','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
            else
                 hReg2(6) = plot(yy,xx,'o','linewidth',linewidth,'linestyle',linestyle,'color',wisColor);
            end
        end
    end
end
